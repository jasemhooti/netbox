...
function ip_in_range($ip, $range){
    ...
    // $range is in IP/CIDR format eg 127.0.0.1/22
...
function getAutoPaymentVerifySettings() {
    global $connection;
    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'AUTO_PAYMENT_VERIFY'");
    $stmt->execute();
    $result = $stmt->get_result()->fetch_assoc();
    if(!is_null($result['value'])) {
        return json_decode($result['value'], true);
    }
    return ['enabled' => 'off', 'minutes' => 30];
}

function updateAutoPaymentVerifySettings($enabled, $minutes) {
    global $connection;
    $settings = json_encode(['enabled' => $enabled, 'minutes' => $minutes]);
    $stmt = $connection->prepare("UPDATE `setting` SET `value` = ? WHERE `type` = 'AUTO_PAYMENT_VERIFY'");
    $stmt->bind_param("s", $settings);
    return $stmt->execute();
}

function checkAndAutoVerifyPayments() {
    global $connection;
    $settings = getAutoPaymentVerifySettings();
    if($settings['enabled'] !== 'on') return;
    
    $minutes = $settings['minutes'];
    $stmt = $connection->prepare("SELECT * FROM `pays` WHERE `state` = 'pending' AND `type` = 'card2card' AND TIMESTAMPDIFF(MINUTE, `date`, NOW()) >= ?");
    $stmt->bind_param("i", $minutes);
    $stmt->execute();
    $result = $stmt->get_result();
    
    while($row = $result->fetch_assoc()) {
        $stmt = $connection->prepare("UPDATE `pays` SET `state` = 'verified' WHERE `payid` = ?");
        $stmt->bind_param("i", $row['payid']);
        $stmt->execute();
        
        // ارسال پیام به کاربر
        $message = "✅ پرداخت شما با موفقیت تایید شد.\nمبلغ: " . $row['amount'] . " تومان";
        sendMessage($message, null, "MarkDown", $row['user_id']);
        
        // ارسال پیام به ادمین
        $adminMessage = "🔔 پرداخت خودکار تایید شد:\nکاربر: " . $row['user_id'] . "\nمبلغ: " . $row['amount'] . " تومان";
        sendMessage($adminMessage, null, "MarkDown", $admin_id);
    }
}
